...
export default function AdminAddProduct() {
  ...
  const Input = ({ ...props }) => (
    <input
      className="w-full px-3.5 py-2.5 border border-surface-300 dark:border-surface-600 rounded-xl bg-white dark:bg-surface-800 text-surface-900 dark:text-surface-100 placeholder:text-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 text-sm"
      {...props}
    />
  );
    ...
    <div className="p-6 max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 border border-surface-300 dark:border-surface-600 rounded-xl bg-white dark:bg-surface-800 text-surface-900 dark:text-surface-100 placeholder:text-surface-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 text-sm resize-none"
                rows={4}
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 border border-surface-300 dark:border-surface-600 rounded-xl bg-white dark:bg-surface-800 text-surface-900 dark:text-surface-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all duration-200 text-sm"
                required
              >
        ...
        <SectionCard title="Pricing & Inventory">
          ...
          {form.originalPrice && form.price && Number(form.originalPrice) > Number(form.price) && (
            <div className="flex items-center gap-2 text-sm text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-3 py-2 rounded-xl">
              <span>✓</span>
              <span>
                {Math.round(((Number(form.originalPrice) - Number(form.price)) / Number(form.originalPrice)) * 100)}% discount will be applied
              </span>
            </div>
          )}
        </SectionCard>
        ...
        <SectionCard title="Product Images">
          <p className="text-xs text-surface-500 dark:text-surface-400 -mt-2">Add image URLs (first image will be the main display image)</p>
          ...
          <button
            ...
            className="text-primary-600 dark:text-primary-400 text-sm flex items-center gap-1.5 hover:underline transition-colors font-medium"
          >
            <FiPlus size={14} /> Add Another Image
          </button>
        </SectionCard>

        <SectionCard title="Tags & Labels">
          <div>
            <Label>Tags <span className="text-surface-400 font-normal">(comma separated)</span></Label>
            <Input
              value={form.tags}
              onChange={e => set('tags', e.target.value)}
              placeholder="wireless, bluetooth, noise-cancelling, premium"
            />
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-4 mt-1">
              {[
                ['isFeatured', 'Featured', 'Shown on homepage featured section'],
                ['isBestSeller', 'Best Seller', 'Mark as best selling product'],
        ...
        <SectionCard title="Specifications">
          <p className="text-xs text-surface-500 dark:text-surface-400 -mt-2">Add key-value pairs for product specifications</p>
        ...
        <div className="flex gap-3 pb-6">
          <button
            type="submit"
            disabled={loading}
            className="flex-1 inline-flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 active:bg-primary-800 text-white font-semibold px-6 py-3 rounded-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
          >
            {loading ? (
              <>
          ...
          <button
            type="button"
            onClick={() => navigate('/admin/products')}
            className="px-6 py-3 border-2 border-surface-300 dark:border-surface-600 text-surface-600 dark:text-surface-400 font-semibold rounded-xl hover:bg-surface-50 dark:hover:bg-surface-700 transition-all duration-200"
          >
            Cancel
          </button>
        </div>
