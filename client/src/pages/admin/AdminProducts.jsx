  ...
  return (
    <div className="p-4 md:p-6 space-y-4 md:space-y-5">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-surface-900 dark:text-surface-50">Products</h1>
        ...
        <>
                ...
                <tbody className="divide-y divide-surface-100 dark:divide-surface-700">
                  {products.length === 0 ? (
                    ...
                    <tr key={p._id} className="hover:bg-surface-50 dark:hover:bg-surface-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-surface-600 dark:text-surface-400 text-sm">{p.category?.name || '—'}</td>
                      <td className="px-4 py-3">
                        <div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-xs text-surface-400 dark:text-surface-500">({p.numReviews || 0})</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <ActionButtons id={p._id} onDelete={handleDelete} />
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div className="md:hidden space-y-3">
            {products.length === 0 ? (
              ...
              <div key={p._id} className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                      <div>
                        ...
                        {p.originalPrice > p.price && (
                          <span className="text-xs text-surface-400 dark:text-surface-500 line-through ml-1.5">₹{p.originalPrice?.toLocaleString()}</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-surface-600 dark:text-surface-300 font-medium">{p.ratings?.toFixed(1) || '—'}</span>
            ...
            {pages > 1 && (
              <div className="bg-white dark:bg-surface-800 rounded-2xl border border-surface-200 dark:border-surface-700 px-4 py-3">
                <Pagination page={page} pages={pages} onPageChange={setPage} />
              </div>
            )}
          </div>
...
function ProductThumb({ product: p, size = 'sm' }) {
