...
import { FiArrowRight, FiZap, FiTruck, FiShield, FiHeadphones, FiStar } from 'react-icons/fi';
import API from '../services/api';
import ProductCard from '../components/product/ProductCard';
import { ProductSkeleton } from '../components/ui/index';

const FEATURES = [
  { icon: <FiTruck className="text-primary-600 dark:text-primary-400" size={20} />,    title: 'Free Delivery',         desc: 'On orders above ₹499',       bg: 'bg-primary-50 dark:bg-primary-950/60' },
  { icon: <FiShield className="text-emerald-600 dark:text-emerald-400" size={20} />,   title: 'Secure Payments',        desc: 'Razorpay + COD available',   bg: 'bg-emerald-50 dark:bg-emerald-950/60' },
  { icon: <FiZap className="text-amber-500 dark:text-amber-400" size={20} />,          title: 'AI Recommendations',     desc: 'Personalised just for you',  bg: 'bg-amber-50 dark:bg-amber-950/60' },
  { icon: <FiHeadphones className="text-blue-600 dark:text-blue-400" size={20} />,     title: '24/7 Support',           desc: 'Live chat always available', bg: 'bg-blue-50 dark:bg-blue-950/60' },
];
...
export default function HomePage() {
  ...
  return (
    <div>
      {/* ── Hero Banner ── */}
      <section className="relative overflow-hidden bg-gradient-to-br from-primary-900 via-primary-800 to-primary-700 dark:from-surface-950 dark:via-primary-950 dark:to-primary-900">
        ...
        <div className="page-container py-16 md:py-24 flex flex-col md:flex-row items-center gap-12 relative z-10">
          <div className="flex-1 space-y-6">
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm border border-white/20 px-4 py-2 rounded-full text-sm font-semibold text-white/90">
            ...
            <h1 className="font-display text-4xl md:text-6xl font-black text-white leading-[1.05] tracking-tight">
              Shop Smarter<br />
              with <span className="text-primary-300">SmartCart</span>
            </h1>

            <p className="text-primary-200 text-lg max-w-lg leading-relaxed">
            ...
            <div className="flex gap-4 flex-wrap pt-2">
              <Link
                to="/products"
                className="inline-flex items-center gap-2 bg-white text-primary-800 font-bold px-7 py-3.5 rounded-xl hover:bg-primary-50 transition-all shadow-sm hover:shadow-md active:scale-[.98]"
              >
              ...
              {!user && (
                <Link
                  ...
                  className="inline-flex items-center gap-2 border-2 border-white/40 text-white font-bold px-7 py-3.5 rounded-xl hover:bg-white/10 transition-all"
                >
                  Create Account
                </Link>
              )}
            ...
            <div className="flex gap-8 pt-4">
              {[['50K+', 'Happy Customers'], ['1K+', 'Products'], ['100%', 'Secure']].map(([val, lbl]) => (
                <div key={lbl}>
                  ...
                  <p className="text-primary-300 text-xs font-medium">{lbl}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex-1 flex justify-center">
            <div className="relative">
              ...
              <div className="absolute -bottom-4 -left-4 bg-white dark:bg-surface-800 px-4 py-2.5 rounded-2xl shadow-lg text-sm font-semibold text-surface-800 dark:text-surface-100 flex items-center gap-2">
      ...
      {/* ── Feature strip ── */}
      <section className="bg-white dark:bg-surface-900 border-b border-surface-100 dark:border-surface-700 shadow-sm">
          ...
          {FEATURES.map(f => (
            <div key={f.title} className="flex items-center gap-3 p-3 rounded-xl hover:bg-surface-50 dark:hover:bg-surface-800 transition-colors">
              ...
              <div>
                ...
                <p className="text-xs text-surface-500 dark:text-surface-400">{f.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="page-container py-12 space-y-16">
          ...
          <section>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-10 gap-3 mt-5">
              {categories.slice(0, 10).map(cat => (
                ...
                >
                  <div className="w-12 h-12 rounded-xl overflow-hidden ring-2 ring-transparent group-hover:ring-primary-300 dark:group-hover:ring-primary-600 transition-all">
                    <img
        ...
        {!user && (
          <section className="rounded-2xl overflow-hidden relative">
            <div className="absolute inset-0 bg-gradient-to-br from-primary-700 to-primary-900 dark:from-primary-900 dark:to-surface-950" />
            <div className="absolute inset-0 opacity-10"
              style={{ backgroundImage: 'radial-gradient(circle, rgba(124,199,195,0.6) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
            <div className="relative z-10 p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <Link
                to="/login?mode=register"
                className="flex-shrink-0 bg-white text-primary-800 font-bold px-8 py-3.5 rounded-xl hover:bg-primary-50 transition-all shadow-sm hover:shadow-md active:scale-[.98] inline-flex items-center gap-2"
              >
  ...
  return (
    <div className="flex items-center justify-between mb-1">
      <div className="flex items-center gap-3">
        ...
        {badge && <span className="badge-primary text-[10px]">{badge}</span>}
      </div>
      <Link
        to={link}
...
function ProductGrid({ products, loading }) {
  ...
  if (!products?.length) return (
    <p className="text-surface-400 dark:text-surface-500 text-sm mt-4">No products to show right now.</p>
  );
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
