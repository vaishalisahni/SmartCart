...
import {
  ...
  FiShoppingCart, FiMenu, FiX, FiUser,
} from 'react-icons/fi';
...
const links = [
  { to: '/seller',          label: 'Dashboard', icon: <FiGrid size={17} />,        end: true },
  { to: '/seller/products', label: 'My Products', icon: <FiShoppingBag size={17} /> },
  { to: '/seller/orders',   label: 'My Orders',   icon: <FiPackage size={17} /> },
];
...
export default function SellerLayout() {
  ...
  const SidebarContent = () => (
    <>
      <div className="p-5 border-b border-primary-900/80">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-primary-400">Seller Panel</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-3 border-t border-primary-900/80 space-y-0.5">
        <NavLink
          to="/"
          className="flex items-center gap-3 px-3 py-2.5 text-sm text-primary-300 hover:text-white hover:bg-primary-900/60 rounded-xl transition-all"
        >
  ...
  return (
    <div className="flex h-screen bg-surface-50 dark:bg-surface-950 overflow-hidden">
      ...
      {/* Mobile Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-60 z-50 bg-primary-950 text-white flex flex-col border-r border-primary-900 transform transition-transform duration-300 md:hidden ${
